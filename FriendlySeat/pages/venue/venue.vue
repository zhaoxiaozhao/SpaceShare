		...
		<view class="card status-row">
			...
			<view class="stat">
				<view class="stat-act" @click="openWaitlist">
					<view class="icon-waitlist" :style="waitlistIconStyle"></view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			fullscreenIconStyle() {
				return this.seasonIconBg(`<path fill='none' stroke='${this.seasonPrimary}' stroke-width='1.8' stroke-linecap='round' stroke-linejoin='round' d='M8 3v5H3M21 8h-5V3M16 21v-5h5M3 16h5v5'/>`)
			},
			floorTabs() {
		...
		methods: {
			...
			resolveExpireAt() {
				const option = this.wlExpireOptions[this.wlExpireIndex]
				const now = new Date()
				if (option === '今天结束') {
			...
			applyWaitlist() {
				...
				if (!expireAt) {
					uni.showToast({ title: '请选择候补结束时间', icon: 'none' })
					return
				}
				if (expireAt.getTime() <= Date.now()) {
					uni.showToast({ title: '结束时间需晚于当前时间', icon: 'none' })
					return
				}
				if (expireAt.getTime() > Date.now() + 24 * 60 * 60 * 1000) {
					uni.showToast({ title: '候补结束时间最长 24 小时', icon: 'none' })
					return
				}
