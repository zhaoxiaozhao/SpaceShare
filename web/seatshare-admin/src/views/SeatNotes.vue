<template>
  <div>
    <div class="toolbar">
      ...
      <span class="tip">被举报的便签会自动隐藏，通过后恢复展示，驳回则删除</span>
    </div>

    <el-table :data="list" border stripe>
      <el-table-column prop="id" label="ID" width="70" />
      <el-table-column prop="seatCode" label="座位" width="150" />
      <el-table-column prop="content" label="内容" min-width="240" show-overflow-tooltip />
      <el-table-column label="作者" width="140">
        <template #default="{ row }">{{ row.ownerName || ('#' + row.seatId) }}</template>
      </el-table-column>
...
async function load() {
  try {
    list.value = await seatNoteApi.list(status.value)
  } catch (e) {}
}
...
async function review(row, approve) {
  try {
    if (!approve) {
      await ElMessageBox.confirm('驳回将删除该便签，确定吗？', '驳回', { type: 'warning' })
    }
    await seatNoteApi.review(row.id, approve)
